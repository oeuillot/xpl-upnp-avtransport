...
class Engine {
	...
	processXplMessage(message, callback) {
		...
		let deviceName = reg[1];
		let instanceId = reg[2];
		let prop = reg[3];
		...
		Async.forEachOf(this._avTransports, (device, deviceKey, callback) => {
			if (!dn.test(device.asun)) {
				callback();
				return;
			}
			...
			device.processXplMessage(message, prop, instanceId, (error) => {
				if (error) {
					console.error(error);
				}
