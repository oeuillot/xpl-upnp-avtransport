	...
	_processRequest(request, response, body) {
		...
		if (nt === "upnp:event" && sid) {
			...
			for (var k in this._upnpServices) {
				...
				if (device.usn.replace(/:/g, '-') !== pathname) {
					continue;
				}

				device.processEvent(body, request, nt, sid, (error) => {
					if (error) {
...
// ex.device = s;
		...
		Async.forEachOf(this._upnpServices, (device, deviceKey, callback) => {
			...
			device.processXplMessage(message, instanceId, sub, (error) => {
				if (error) {
