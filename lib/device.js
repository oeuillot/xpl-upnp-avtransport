...
class Device {
	...
	_requestUpnpService(instanceID, ns, controlURL, action, body, callback) {
      ...
      ${body || ''} \
		...
		debug("requestPosition", "InstanceID=", instanceID, "Params=", params, "body=", body);
		Request(params, (error, response, body) => {
			if (error) {
				console.error(error);
	...
	_processVolumeChanged(instanceID, channel, volume, callback) {
		...
		var body = this._fillBody(instanceID, "Volume", volume);
	...
	_updateVolume(instanceID, channel, callback) {
		...
		this.requestVolume(instanceID, channel, (error, xml2) => {
			...
			if (volume && volume !== this._currentVolumes[channel]) {
				cmds.push((callback) => {
