...
class Device {
	 ...
	 * @param address
	 */
	constructor(engine, usn, headers, address, asun) {
		debug("Device", "New device usn=", usn, "address=", address);
	...
	connect(headers, statusCode, address) {
		...
		Request.get(headers.LOCATION, (error, response, body) => {
			...
			var eurl = Url.resolve(headers.LOCATION, avt.valueWithPath("eventSubURL"));
			this._eventURL = eurl;

			var curl = Url.resolve(headers.LOCATION, avt.valueWithPath("controlURL"));
			this._controlURL = curl;
	...
	_subscribe(callback) {
		...
		Request(params, (error, response, body) => {
			...
			this._installGetPositionInterval();
			callback();
		});
	}
	...
	requestPositionInfo(instanceID, callback) {
		this._requestAVTtransport(instanceID, "urn:schemas-upnp-org:service:AVTransport:1", "GetPositionInfo", callback);
	}

	requestMediaInfo(instanceID, callback) {
		this._requestAVTtransport(instanceID, "urn:schemas-upnp-org:service:AVTransport:1", "GetMediaInfo", callback);
	}

	requestTransportInfo(instanceID, callback) {
		this._requestAVTtransport(instanceID, "urn:schemas-upnp-org:service:AVTransport:1", "GetTransportInfo", callback);
	}

	_requestAVTtransport(instanceID, ns, action, body, callback) {
      ...
      ${body || ''} \
		...
		//debug("requestPosition", "InstanceID=", instanceID, "Params=", params);
		Request(params, (error, response, body) => {
			if (error) {
				console.error(error);
	...
	_processStateChanged(instanceID, currentState, callback) {
		...
		var body = this._fillBody("TransportState", currentState);
	...
	_processCurrentPlayModeChanged(instanceID, currentPlayMode, callback) {
		...
		var body = this._fillBody("CurrentPlayMode", currentPlayMode);
	...
	_processCurrentTrackChanged(instanceID, currentTrack, callback) {
		...
		var body = this._fillBody("CurrentTrack", currentTrack);
	...
	_processCurrentTrackDurationChanged(instanceID, currentTrackDuration, callback) {
		...
		var body = this._fillBody("CurrentTrackDuration", currentTrackDuration);
	...
	_processURIChanged(instanceID, uri, callback) {
		...
		var body = this._fillBody("AVTransportURI", uri);
	...
	_installGetPositionInterval() {
		this._getPositionIntervalId = setInterval(() => {
	...
	stop(instanceID, callback) {
		this._requestAVTtransport(instanceID, "urn:schemas-upnp-org:service:AVTransport:1", "Stop", callback);
	}

	pause(instanceID, callback) {
		this._requestAVTtransport(instanceID, "urn:schemas-upnp-org:service:AVTransport:1", "Pause", callback);
	}
	...
	play(instanceID, speed, callback) {
		...
		this._requestAVTtransport(instanceID, "urn:schemas-upnp-org:service:AVTransport:1", "Play", `<Speed>{speed}</Speed>`, callback);
	}
	...
	setVolume(instanceID, channel, desiredVolume, callback) {
		...
		this._requestAVTtransport(instanceID, "urn:schemas-upnp-org:service:AVTransport:1", "Volume", `<Channel>{channel}</Channel><DesiredVolume>{desiredVolume}</DesiredVolume>`, callback);
	}
