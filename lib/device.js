...
class Device {
	...
	processEvent(body, request, nt, sid, callback) {
		...
		this._propertiesChanged(xml2, instanceID, EVENT_PROPS, true, (xml2, changes) => {
			this._updateMetaDatas(changes, changes['currentTrackMetaData']);
			this._updateMetaDatas(changes, changes['AVTransportURIMetaData']);

			let volume = xml2.valueWithPath("Volume@val");
			if (volume !== undefined) {
				let channel = xml2.valueWithPath("Volume@channel") || 'Master';
	...
	_propertiesChanged(xml2, instanceID, props, ignoreIfUndefined, postChange, callback) {
		if (!xml2) {
			return callback();
		}
	...
	_updateVolume(instanceID, channel, callback) {
		...
		this.requestVolume(instanceID, channel, (error, xml2) => {
			...
			this._propertiesChanged(xml2, instanceID, props, false, null, callback);
		});
	}
	...
	_updatePositionInfo(instanceID, callback) {
		this.requestPositionInfo(instanceID, (error, xml2) => {
			...
			this._propertiesChanged(xml2, instanceID, POSITION_INFOS_PROPS, false, (xml2, changes) => {
				this._updateMetaDatas(changes, changes['currentTrackMetaData']);
			}, callback);
		});
	}
	...
	_updateMediaInfo(instanceID, callback) {
		this.requestMediaInfo(instanceID, (error, xml2) => {
			...
			this._propertiesChanged(xml2, instanceID, MEDIA_INFOS_PROPS, false, (xml2, changes) => {
				this._updateMetaDatas(changes, changes['AVTransportURIMetaData']);
			}, callback);
		});
	}
	...
	_updateTransportInfo(instanceID, callback) {
		this.requestTransportInfo(instanceID, (error, xml2) => {
			...
			this._propertiesChanged(xml2, instanceID, TRANSPORT_INFOS_PROPS, false, null, callback);
		});
	}
