...
class Device {
	...
	_updateMediaInfo(instanceID, callback) {
		this.requestMediaInfo(instanceID, (error, xml2) => {
			...
			var uri = xml2.valueWithPath("MediaDuration");
			if (uri && uri !== this._currentURI) {
				cmds.push((callback) => {
					this._processMediaDurationChanged(instanceID, uri, callback);
				});
			}
			...
			var currentTrackMetaData = xml2.valueWithPath("TrackMetaData");
			if (currentTrackMetaData && currentTrackMetaData !== this._currentTrackMetaData) {
				cmds.push((callback) => {
					this._processCurrentTrackMetaDataChanged(instanceID, currentTrackMetaData, callback);
				});
			}
