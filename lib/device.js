...
class Device {
	...
	processEvent(body, request, callback) {
		...
		this._propertiesChanged(instanceID, EVENT_PROPS, callback);
	}
	...
	_updateMediaInfo(instanceID, callback) {
		this.requestMediaInfo(instanceID, (error, xml2) => {
			...
			this._propertiesChanged(xml2, instanceID, MEDIA_INFO_PROPS, callback);
		});
	}
	...
	_updateTransportInfo(instanceID, callback) {
		this.requestTransportInfo(instanceID, (error, xml2) => {
			...
			this._propertiesChanged(xml2, instanceID, TRANSPORT_INFO_PROPS, callback);
		});
	}
