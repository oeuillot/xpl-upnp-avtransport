...
class Device {
	...
	requestVolume(instanceID, channel, callback) {
		...
		this._requestUpnpService(instanceID, RENDERING_USN, this._renderingControlURL, "GetVolume", body, callback);
	}
	...
	processEvent(body, request, nt, sid, callback) {
		...
		this._propertiesChanged(xml2, instanceID, AVTRANSPORT_EVENT_PROPS, true, "upnp.AVTransport", (xml2, changes) => {
			this._updateMetaDatas(changes, changes['currentTrackMetaData']);
			this._updateMetaDatas(changes, changes['AVTransportURIMetaData']);

		}, (error) => {

			this._propertiesChanged(xml2, instanceID, RENDERING_CONTROL_EVENT_PROPS, true, "upnp.RenderingControl", (xml2, changes) => {
	...
	_propertiesChanged(xml2, instanceID, props, ignoreIfUndefined, postChange, bodyName, callback) {
		if (!xml2) {
			return callback();
		}
		...
		Async.eachOfSeries(changes, (value, name, callback) => {
			...
			xpl.sendXplStat(body, bodyName, callback);
	...
	_updateVolume(instanceID, channel, callback) {
		...
		this.requestVolume(instanceID, channel, (error, xml2) => {
			...
			this._propertiesChanged(xml2, instanceID, props, false, null, "upnp.RenderingControl", callback);
		});
	}
	...
	_updatePositionInfo(instanceID, callback) {
		this.requestPositionInfo(instanceID, (error, xml2) => {
			...
			this._propertiesChanged(xml2, instanceID, POSITION_INFOS_PROPS, false, "upnp.AVTransport", (xml2, changes) => {
				this._updateMetaDatas(changes, changes['currentTrackMetaData']);
			}, callback);
		});
	}
	...
	_updateMediaInfo(instanceID, callback) {
		this.requestMediaInfo(instanceID, (error, xml2) => {
			...
			this._propertiesChanged(xml2, instanceID, MEDIA_INFOS_PROPS, false, "upnp.AVTransport", (xml2, changes) => {
				this._updateMetaDatas(changes, changes['AVTransportURIMetaData']);
			}, callback);
		});
	}
	...
	_updateTransportInfo(instanceID, callback) {
		this.requestTransportInfo(instanceID, (error, xml2) => {
			...
			this._propertiesChanged(xml2, instanceID, TRANSPORT_INFOS_PROPS, false, null, "upnp.AVTransport", callback);
		});
	}
		...
		this.prepareForConnection(protocolInfo, (error, result) => {
			...
			this.setAVTransportURI(instanceId, url, metadatas, (error) => {
				...
				if (message.uuid) {
					...
					xpl.sendXplStat(body, "upnp.avtransport", callback);
					return;
				}
	...
	setVolume(instanceID, channel, desiredVolume, callback) {
		...
		this._requestUpnpService(instanceID, RENDERING_USN, this._renderingControlURL, "SetVolume", body, callback);
	}
