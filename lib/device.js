...
class Device {
	...
	_updatePositionInfo(instanceID, callback) {
		this.requestPositionInfo(instanceID, (error, xml2) => {
			...
			/*
			 CurrentTrackMetaData/<DIDL-Lite xmlns:dc="http://purl.org/dc/elements/1.1/"
			 xmlns:upnp="urn:schemas-upnp-org:metadata-1-0/upnp/"
			 xmlns="urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/">
			 <item id="57402b486f1b2b3b6ae5aaca"><upnp:class>object.item.audioItem.musicTrack</upnp:class><dc:title>Celebrate</dc:title><upnp:artist>Mika</upnp:artist><upnp:album>W9 Hits 2013</upnp:album><upnp:originalTrackNumber>2</upnp:originalTrackNumber><upnp:genre>Compilation</upnp:genre><res size="7547212" bitrate="320000" nrAudioChannels="2" sampleFrequency="44100" duration="0:03:07" protocolInfo="http-get:*:audio/mpeg:*">http://192.168.3.193:10293/cds/content/57402b486f1b2b3b6ae5aaca</res></item></DIDL-Lite>]
			 */
			...
			this._propertiesChanged(xml2, instanceID, POSITION_INFOS_PROPS, (error, changes) => {
				...
				let ctmd = changes['CurrentTrackMetaData'];
				if (ctmd && /^<DIDL-LITE/.exec(ctmd)) {
					try {
						let xml2 = new Xmldoc.XmlDocument(ctmd);
						title = xml2.valueWithPath("item.title");
						artist = xml2.valueWithPath("item.artist");
						genre = xml2.valueWithPath("item.genre");
					} catch (x) {
						console.error(x);
					}
				}
				...
				this._addPropertyChanged(cmds, instanceID, "TrackTitle", title);
				this._addPropertyChanged(cmds, instanceID, "TrackArtist", artist);
				this._addPropertyChanged(cmds, instanceID, "TrackGenre", genre);
	...
	_updateMediaInfo(instanceID, callback) {
		this.requestMediaInfo(instanceID, (error, xml2) => {
			...
			this._propertiesChanged(xml2, instanceID, MEDIA_INFOS_PROPS, callback);
		});
	}
